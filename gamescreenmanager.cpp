...
QPoint GameScreenManager::findPixel(QColor clr, int redNoise, int greenNoise, int blueNoise, QPoint hintPoint, QSize hintSize)
{
	QScreen *screen = QGuiApplication::primaryScreen();
	QPixmap screenShotPixmap = screen->grabWindow(0);
	QImage image = screenShotPixmap.toImage();
	...
	if(!hintPoint.isNull() && !hintSize.isEmpty())
	{
		beginX = qMax(0, hintPoint.x() - hintSize.width() / 2) * ratioH;
		endX = qMin(image.width(), hintPoint.x() + hintSize.width() / 2) * ratioH;
		beginY = qMax(0, hintPoint.y() - hintSize.height() / 2) * ratioV;
		endY = qMin(image.height(), hintPoint.y() + hintSize.height() / 2) * ratioV;
	}

	qDebug() << "findPixel " << beginX << endX << beginY << endY;
