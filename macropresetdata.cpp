...
public:
	...
	void run() Q_DECL_OVERRIDE
	{
		...
		MacroAction::ActionType currBlockStartType;
		bool firstLoop = true;
		while(!m_stop)
		{
			...
			if(firstLoop || index < 0 || index >= m_owner->m_model.count())
			{
				...
				lastLoopEndIndex = -1;
				index = 0;	// go to first
				m_owner->m_maxIndex = 0;
				m_owner->increaseLoopCount();
			...
			{
				currBlockStartType = (MacroAction::ActionType)action->actionType();
			...
			if(action->doit())
			{
				...
				if(action->actionType() == MacroAction::Else)
				{
			...
			else
			{
				...
				LOG_INFO() << "MACRO ACTION RUNNING => OK : " << index << " : " << action->description() << ", JUMP :" << needToJumpToEnd;
			}
			...
			if(needToJumpToEnd)
			{
				...
				if(lastLoopEndIndex < 0 || index > lastLoopEndIndex)
				{
					...
					for(; index < m_owner->m_model.count(); index++)
					{
						...
						{
							// Find a else type
							if(tempAction->actionType() == MacroAction::Else)
							{
								lastLoopEndIndex = index;
								break;
							}
...
MacroPresetData::MacroPresetData(MacroPresetListModel *model) : QObject(NULL)
  ...
  , m_model(this), m_currentIndex(0), m_repeat(true), m_maxIndex(-1), m_hangupTickCount(0)
...
void MacroPresetData::onHangupCheckTimer(void)
{
	...
	//qDebug() << m_currentIndex << m_maxIndex << m_hangupTickCount;
	...
	if(m_hangupTickCount >= kHangupTimeout)
	{
...
void MacroPresetData::goToMainScreen(void)
{
	...
	// go to game menu
	while(!Lazybones::gameScreenManager()->isCurrentScreen(GameScreenData::GameMenu, true))
	{
