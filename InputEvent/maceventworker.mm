...
public:
	...
	bool nativeEventFilter(const QByteArray &eventType, void *message, long *result)
	{
			...
			//		case NSKeyUp:
		{
			...
			QKeyEvent ev(QEvent::KeyPress, convertToQtKey(keyCode), Qt::NoModifier);
...
{
	qDebug() << "AXIsProcessTrusted : " << AXIsProcessTrusted();
...
void MacEventManager::stopFiltering(void)
{
	m_hookBitsEnabled = 0;
}
...
void MacEventManager::doKeyboardInput(Qt::Key keyCode)
{
	...
	qDebug() << keyCode << convertToMacKey(keyCode);
